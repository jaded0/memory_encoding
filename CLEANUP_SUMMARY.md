...
This document summarizes the unused/dead code that was removed from `ephemeral_model.py` and `train.py`.

## Removed from `ephemeral_model.py`:
...
### 2. **self.imprints parameter** (EphemeralLinear.__init__)
- **Status**: REMOVED
...
### 3. **self.plasticity_candidate_weights** (EphemeralLinear.__init__)
- **Status**: REMOVED
- **Reason**: Created with `torch.zeros_like(self.weight)` but never referenced
- **Impact**: No functional impact, reduces memory usage

### 4. **self.frequency parameter** (EphemeralLinear.__init__)
- **Status**: REMOVED
...
### 5. **self.phase_shift parameter** (EphemeralLinear.__init__)
- **Status**: REMOVED
- **Reason**: Had initialization logic but was never used in any computations
- **Impact**: No functional impact, reduces memory usage

### 6. **self.learning_rate = 1** (EphemeralLinear.__init__)
- **Status**: REMOVED
- **Reason**: Set but never referenced anywhere in the code
- **Impact**: No functional impact

### 7. **update_weights method** (EphemeralLinear class, lines ~183-262)
- **Status**: REMOVED
...
### 8. **update_weights_dfa method** (EphemeralRNN class)
- **Status**: REMOVED
...
- **Status**: REMOVED
- **Reason**: Fallback case for "non-Ephemeral models" that would never execute in practice since the codebase only uses EphemeralRNN (unified approach) or SimpleRNN (no Hebbian methods)
...
- ✅ Both `ephemeral_model.py` and `train.py` import successfully after cleanup
...
1. `ephemeral_model.py` - Removed unused class, parameters, and methods
2. `train.py` - Removed unreachable fallback code
