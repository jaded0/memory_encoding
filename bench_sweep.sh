...
#SBATCH --array=0-299%20       # 300 total jobs (1*2*2*5*3*5), max 15 concurrent
...
declare -a updaters=('backprop' 'rnn')
declare -a enable_recurrence=('true' 'false')
declare -a clip_weights=('0' '1' '10' '1e-1' '1e-2')
declare -a plast_clip=('1e4' '1e3' '1e2')
declare -a grad_clip=('0' '1e-1' '10' '1' '1e-2')
...
total_jobs=$((num_model_types * num_updaters * num_enable_recurrence * num_clip_weights * num_plast_clip * num_grad_clip))
...
# Order: grad_clip -> plast_clip -> clip_weights -> enable_recurrence -> updater -> model_type
...
GROUP='bench_sweep'
RUN_NOTES="Model=${MODEL_TYPE}_Updater=${UPDATER}_Recurrence=${ENABLE_RECURRENCE}_ClipWeights=${CLIP_WEIGHTS}_PlastClip=${PLAST_CLIP}_GradClip=${GRAD_CLIP}_TaskID=${SLURM_ARRAY_TASK_ID}"
