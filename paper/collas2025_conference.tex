...
\hline
\end{tabular}
\caption{Performance comparison across model configurations. Ethereal models maintain per-batch-item weight sets, enabling memory storage without recurrence. The success of ethereal weights with DFA stems from the stability provided by fixed random feedback weights, while backpropagation configurations work but with higher volatility due to the high-plasticity weights being used in gradient calculations.}
...
A key early warning indicator emerges from monitoring the gradient norms of high-plasticity and low-plasticity weights. Stability correlates with this value shrinking and stabilizing over time, while growth of the gradient norm precedes explosive behavior (Figure 6). This observation aligns with the intuitive understanding that high-plasticity weights, when left unchecked by slower stabilizing weights, can enter a positive feedback loop where rapid changes amplify subsequent gradient signals.
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.8\linewidth]{compare_norms.png}
...
Holding other hyperparameters constant, varying the base learning rate and the plasticity modifier reveals a linear boundary between stable and unstable regimes. Lower base learning rates allow for higher plasticity modifiers before instability arises, while higher base learning rates necessitate more conservative plasticity settings (Figure 7). A limitation of this experiment is that some runs that initially appear stable within the training timeframe still exhibit climbing loss numbers at the tail end, suggesting eventual instability if training were extended further. 
\begin{figure}[h]
\begin{center}
