...
\subsection{Activatiohn-Based Memory Mechanisms}
...
Large Language Models (LLMs) have surpassed human performance on a variety of tasks, yet progress has largely stalled on their performance on many things routinely done by even a household cat. Such failure modes include long-term coherence in an agentic task, quick incorporation of new information, management of memory over long conversations, and robustness to adversarial attacks such as camoflage, all hinging on the same shortcoming: the lack of a functional progression of state over multiple timescales. A biological system, such as that of a simple mammal, has countless mechanisms for storing memory, persisting state, and managing plasticty that current LLMs only crudely approximate with its only two available systems: parameter-based memory and activation-based memory. Rather than exclusively relying on the former for long-term knowledge and the latter for short-term context, ethereal weights present a more fluid memory system.
