...
| 2026-09-23 | this commit (see `git log -- tests/fixtures/training_traces.json`) | Renames only; values identical. Event key `unified_update` → `update` (the method is now `apply_update`). Checked by renaming that key in the previous fixture and comparing: equal, and byte-identical when dumped |
